...
export async function POST(request: Request) {
  try {
    ...
    const sheetId = getSheetId('lifer');
    if (!sheetId) {
      throw new Error('Lifer sheet ID not found');
    }
    ...
    const rowData = [
      ...
      formData.criminalHistory || '',        // 11. Criminal History
      formData.backgroundStory || '',        // 12. Background Story
      formData.sentenceLength || '',         // 13. Sentence Length
      formData.crimeDescription || '',       // 14. Crime Description
      formData.mentalState || '',            // 15. Mental State
      formData.dateOfBirth || '',            // 16. Date of Birth
      formData.address || '',                // 17. Address
      'pending',                             // 18. Status
      ''                                     // 19. Moderator (empty initially)
    ];
